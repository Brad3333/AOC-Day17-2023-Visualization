...
public class Calc {
	...
	// This method calculates the path and minimum heat loss for part 1
	public int minHeatLoss() {
	    ...
	    while (!pq.isEmpty()) {
	        ...
	        // Continue in the same direction (if we haven't moved more than 3 steps in the same direction)
	        if (n < 3) {
	        ...
	        for (int[] direction : directions) {
	            ...
	            if ((ndr != dr || ndc != dc) && !(ndr == -dr && ndc == -dc)) {
	                ...
	                // If the new row and column are valid indexes add them to the priority queue 
	                if (nr >= 0 && nr < grid.length && nc >= 0 && nc < grid[0].length) {
	                    pq.add(new State(hl + grid[nr][nc], nr, nc, ndr, ndc, 1, current));
	                }
